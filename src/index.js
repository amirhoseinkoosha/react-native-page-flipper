...
const PageFlipper = React.forwardRef(({ data, enabled = true, pressable = true, singleImageMode = true, renderLastPage, portrait = false, onFlippedEnd, onFlipStart, onPageDrag, onPageDragEnd, onPageDragStart, onEndReached, onInitialized, renderContainer, renderPage, pageSize = { height: 600, width: 400 }, contentContainerStyle, }, ref) => {
    ...
    const containerSize = useMemo(() => {
        ...
        let size = {
            ...
            width: singleImageMode && !state.isPortrait
                ? pageSize.width * 2
                : pageSize.width,
        };
    ...
    const getAdjustedIndex = (allPages) => {
        ...
        if (previousPortrait !== undefined &&
            ...
            if (portrait) {
                ...
                adjustedIndex = Math.floor(adjustedIndex % 2 === 0
                    ? adjustedIndex / 2
                    : (adjustedIndex - 1) / 2);
            }
        }
    ...
    const onPageFlipped = useCallback((index) => {
        const newIndex = state.nextPageIndex !== undefined
            ? state.nextPageIndex
    ...
    const getPageStyle = (right, front) => {
        ...
        const pageStyle = {
            height: containerSize.height,
            width: singleImageMode && !isPortrait
    ...
    const shouldRenderLastPage = (isSecondToLastPage || isLastPage) &&
        singleImageMode &&
        data.length % 2 !== 0;
    const bookPageProps = {
    ...
    return (React.createElement(View, { style: styles.container, onLayout: onLayout },
        React.createElement(View, { style: [
            ...
            React.createElement(ContentWrapper, null, !isPortrait ? (React.createElement(View, { style: styles.content },
                !prev ? (React.createElement(Empty, null)) : (React.createElement(BookPage, { ref: prevBookPage, right: false, front: current, back: prev, key: `left${pageIndex}`, ...bookPageProps })),
                !next ? (React.createElement(Empty, null)) : (React.createElement(BookPage, { ref: nextBookPage, right: true, front: current, back: next, key: `right${pageIndex}`, ...bookPageProps })),
                React.createElement(BookPageBackground, { left: !prev ? current.left : prev.left, right: !next ? current.right : next.right, isFirstPage: isFirstPage, isLastPage: isLastPage, getPageStyle: getPageStyle, containerSize: containerSize, renderPage: renderPage, renderLastPage: renderLastPage, shouldRenderLastPage: shouldRenderLastPage }))) : (React.createElement(View, { style: styles.portraitContent },
                React.createElement(View, { style: { ...StyleSheet.absoluteFillObject } },
                    React.createElement(BookPagePortrait, { ...bookPageProps, current: current, prev: prev, next: next, onPageFlip: onPageFlipped, key: `right${pageIndex}`, ref: portraitBookPage })),
                next && (React.createElement(View, { style: {
                        ...StyleSheet.absoluteFillObject,
                        zIndex: -5,
                        overflow: 'hidden',
                    } }, renderPage && (React.createElement(View, { style: getPageStyle(true, false) }, renderPage(next.right)))))))))));
});
export default React.memo(PageFlipper);
const Wrapper = (props) => React.createElement(View, { style: styles.wrap, ...props });
const Empty = () => React.createElement(View, { style: styles.container, pointerEvents: "none" });
