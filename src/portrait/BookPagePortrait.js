...
import Animated, { Easing, Extrapolation, interpolate, runOnJS, useAnimatedGestureHandler, useAnimatedStyle, useDerivedValue, useSharedValue, withTiming, } from 'react-native-reanimated';
...
const BookPagePortrait = React.forwardRef(({ current, prev, onPageFlip, containerSize, enabled, isPressable, setIsAnimating, getPageStyle, isAnimating, isAnimatingRef, next, onFlipStart, onPageDrag, onPageDragEnd, onPageDragStart, renderPage, }, ref) => {
    ...
    const getDegreesForX = (x) => {
        'worklet';
        const val = interpolate(x, [-containerSize.width, 0, containerSize.width], [180, 0, -180], Extrapolation.CLAMP);
        return val;
    };
    ...
    return (React.createElement(Animated.View, { style: containerStyle },
        React.createElement(PanGestureHandler, { onGestureEvent: onPanGestureHandler, enabled: gesturesEnabled },
            React.createElement(Animated.View, { style: containerStyle },
                isPressable && prev && (React.createElement(Pressable, { disabled: isAnimating, onPress: () => {
                ...
                isPressable && next && (React.createElement(Pressable, { disabled: isAnimating, onPress: () => {
                        if (!isAnimatingRef.current)
                            turnPage(1);
                    }, style: {
                        position: 'absolute',
                ...
                current && next ? (React.createElement(IPage, { page: current, right: true, ...iPageProps })) : (React.createElement(View, { style: { height: '100%', width: '100%' } }, renderPage && (React.createElement(View, { style: getPageStyle(true, true) }, renderPage(current.right))))),
                prev && (React.createElement(IPage, { page: prev, right: false, ...iPageProps }))))));
});
const IPage = ({ right, page, rotateYAsDeg, containerWidth, containerSize, getPageStyle, renderPage, }) => {
    ...
    return (React.createElement(View, { style: {
        ...
        React.createElement(Animated.View, { style: [
            ...
            React.createElement(View, { style: styles.pageContainer }, renderPage && (React.createElement(Animated.View, { style: [
                    backPageStyle,
                    {
                        opacity: 0.2,
                        transform: [
            ...
            React.createElement(BackShadow, { degrees: rotationVal, right: true }),
            React.createElement(FrontShadow, { ...shadowProps }),
            React.createElement(PageShadow, { ...shadowProps, containerSize: containerSize })),
        React.createElement(Animated.View, { style: [styles.pageContainer, portraitFrontStyle] }, renderPage && (React.createElement(Animated.View, { style: [frontPageStyle] }, renderPage(page.left))))));
};
